...
class Application:
    ...
    def __init__(self):
                ...
                # Initialize the camera, model, and speech engine
        self.vs = cv2.VideoCapture(0)  # Open camera feed
        self.current_image = None
        self.model = load_model('cnn8grps_rad1_model.h5')  # Load trained model
        self.speak_engine = pyttsx3.init()  # Initialize text-to-speech engine
        self.speak_engine.setProperty("rate", 100)  # Set speaking rate
        voices = self.speak_engine.getProperty("voices")  # Get available voices
        self.speak_engine.setProperty("voice", voices[0].id)  # Set default voice

        # Initialize character tracking and flags
        self.ct = {'blank': 0}  # Dictionary to track character counts
        self.blank_flag = 0
        self.space_flag = False
        self.next_flag = True
        self.prev_char = ""
        self.count = -1
        self.ten_prev_char = [" "] * 10  # Track last 10 characters

        # Initialize counts for uppercase alphabet
        ...
        self.root = tk.Tk()
        self.root.title("Sign Language To Text Conversion")  # Set window title
        self.root.protocol('WM_DELETE_WINDOW', self.destructor)  # Set close protocol
        self.root.geometry("1000x600")  # Set window size (reduced)

        # Camera panel to display live feed
        self.panel = tk.Label(self.root)
        self.panel.place(x=100, y=50, width=440, height=440)  # Reduced size of camera feed

        # Additional image panel to display detected signs or results
        self.panel2 = tk.Label(self.root)  # initialize image panel
        self.panel2.place(x=700, y=50, width=440, height=440)  # Reduced size of image panel

        # Title label
        self.T = tk.Label(self.root)
        self.T.place(x=10, y=10)
        self.T.config(text="Sign Language To Text Conversion", font=("Courier", 18, "bold"))  # Reduced font size

        # Current symbol panel
        self.panel3 = tk.Label(self.root)  # Current Symbol
        self.panel3.place(x=200, y=520)

        # Character label
        self.T1 = tk.Label(self.root)
        self.T1.place(x=10, y=520)
        self.T1.config(text="Character :", font=("Courier", 18, "bold"))  # Reduced font size

        # Sentence panel
        self.panel5 = tk.Label(self.root)  # Sentence
        self.panel5.place(x=200, y=560)

        # Sentence label
        self.T3 = tk.Label(self.root)
        self.T3.place(x=10, y=560)
        self.T3.config(text="Sentence :", font=("Courier", 18, "bold"))  # Reduced font size

        # Suggestions label
        self.T4 = tk.Label(self.root)
        self.T4.place(x=10, y=600)
        self.T4.config(text="Suggestions :", fg="red", font=("Courier", 18, "bold"))  # Reduced font size

        # Speak button for text-to-speech output
        self.speak = tk.Button(self.root)
        self.speak.place(x=850, y=550)
        self.speak.config(text="Speak", font=("Courier", 16), wraplength=100, command=self.speak_fun)  # Speak button configuration

        # Clear button to reset the screen or clear results
        self.clear = tk.Button(self.root)
        self.clear.place(x=850, y=480)
        self.clear.config(text="Clear", font=("Courier", 16), wraplength=100, command=self.clear_fun)  # Clear button configuration

        # Additional buttons (you can set their function as needed)
        self.b1 = tk.Button(self.root)
        self.b1.place(x=400, y=600)

        self.b2 = tk.Button(self.root)
        self.b2.place(x=550, y=600)

        self.b3 = tk.Button(self.root)
        self.b3.place(x=700, y=600)

        self.b4 = tk.Button(self.root)
        self.b4.place(x=850, y=600)

        # Initialize variables for storing current and previous words/characters
        self.str = " "
        self.ccc = 0
        self.word = " "
        ...
        # Start the video loop to capture live feed from camera
        self.video_loop()
